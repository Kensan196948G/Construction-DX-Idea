...
| 2026-09-04 | **Gate1-5統合WebUI** | ✅ P0①の残り（#50フォローアップ）を実装。詳細ビュー右カラムに「🚪 Gate1〜5 承認フロー」カードを追加し、Gate1-5の状態・必要Authorityごとの承認状況（承認者/理由/日時）表示と、Gate開始(init)・承認依頼(Gate+Authority+承認者メール)・承認/差戻し/却下(理由)操作を実装。App.tsxブリッジで実API（getGates/initGates/request-approval/approval）へ接続し、詳細表示時と案件選択時にGate状態を自動ロード。dc-runtimeの制約（引数付きonClick式が解決不可）を実測で特定し、判定ボタンをrenderValsのクロージャ方式へ変更（既存の単一承認判定ボタンの同種バグも併せて修正）。`npm run verify` PASS（test 103件）・PlaywrightでモックUI遷移と実DB API（Gate1=3Authority全員承認→approved→Gate2解禁）を確認 |
| 2026-09-04 | **20フェーズ Idea-to-Value 進捗管理** | ✅ Phase A 仕上げ（P0④）。migration 010で`ideas.phase_no/phase_note`＋`idea_phase_history`を追加（既存stageから後方互換初期化）。shared.tsに`ideaValuePhases`（docs/New #04準拠の20定義）等を追加。worker API: `GET/POST /api/ideas/:id/phase`（1段階ずつ前進・管理者のみ後戻し・監査＋履歴）。詳細ビューに「🧭 Idea-to-Value 進捗」カード（Phase x/20バー・現在フェーズ・進捗メモ・20フェーズ一覧・次へ進めるボタン）を追加し、App.tsxブリッジで自動ロード。`npm run verify` PASS（test 107件）・実DB E2E（4→5→6→7前進・2段飛ばし422）とPlaywright（Phase 11→12・一覧✓表示）を確認。本番DB `dx_idea`にも010適用済み |

...
> **適用優先順位**: 本ファイル冒頭の `/goal` 文（1-35行目）が発行済みの場合、その Exit Condition（34行目: Release Ready 判断 / Production Ready 判断 / Goal 達成 / 5時間到達 / 20ターン到達）を終了条件として優先する。以下の v10.0 テンプレートは `/goal` 未指定時の汎用フォールバックであり、104行目の終了条件はこのフォールバック運用時にのみ適用する。
...
`CTO全権委任` が指定された場合のみ有効（1-35行目の `/goal` 文の Authority 節で既に宣言済みの場合はこの時点で成立）。
Supervisor → Workflow Engine → Agent Teams → SubAgents → Monitor → Plan → Execute → Verify → Review → Improve ↺
終了条件: Release Ready / Production Ready / Blocked / 5時間到達 / 20ターン到達（`/goal` 文発行時は34行目が正本）

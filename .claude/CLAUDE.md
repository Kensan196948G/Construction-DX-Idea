...
- PR merge は `.claude/commands/safe-auto-merge.md`（`/safe-auto-merge` コマンド）に従う。default branch 宛は必ず人間の選択式、default branch 以外は CI・review・mergeability・危険ファイル gate 全通過時のみ自動 merge 可
...
- 学習発生時: `learning` 更新
- セッション終了時: 最終状態 Write
- CTO が「デプロイ準備完了」と判断した時: `deploy.ready=true` に更新
- 人間によるデプロイ完了報告を受けた時: `project.phase_mode` を `"development"` から `"maintenance"` に更新（値は `"development"` / `"maintenance"` の 2 種）
...
```json
"hooks": {
  "TeammateIdle": [{ "hooks": [{ "type": "command", "command": "node .claude/claudeos/scripts/hooks/teammate-idle-gate.js" }] }],
...
```
> `TeammateIdle` は matcher 非対応で常に発火する（公式仕様）。`TaskCreated` / `TaskCompleted` は matcher 指定可。
...
0. 現在のブランチ（`git branch --show-current`）が `gh repo view --json defaultBranchRef` で取得した default branch（通常 `main`）と一致するか確認する。一致する場合は手順2の commit までに留め、push と PR 作成（手順3・4）は行わず人間確認待ちとする
1. 現在の作業内容を整理
2. 最小単位で commit
3. push（手順0で default branch ではないことを確認済みの場合のみ）
4. PR 作成（Draft 可、手順0で default branch ではないことを確認済みの場合のみ）
...
| 状態 | 処理 |
|---|---|
| STABLE 達成 | `.claude/commands/safe-auto-merge.md`（`/safe-auto-merge`）に従い merge 判断（default branch 宛は必ず人間へ確認）→ 終了報告。本番デプロイは対象外（常に人間が手動実行） |

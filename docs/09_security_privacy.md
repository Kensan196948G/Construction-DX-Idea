...
## 13. 複数AIプロバイダー（2026-08-12追加）

- Claude（`ANTHROPIC_API_KEY`）とDeepSeek（`DEEPSEEK_API_KEY`）をサポート。
- APIキー本体はCloudflare Secretで管理し、DB・GitHub・WebUI・ブラウザ保存領域に保存しない（従来方針を継続）。
- プロバイダーごとにモデル許可リストを設け、許可外モデルは400で拒否する。
- 監査・AI利用履歴にはプロバイダー・モデルを記録する。

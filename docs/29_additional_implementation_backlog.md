...
- 【P2】匿名提案モード・代理登録
- 【P1】オフライン下書きの IndexedDB 化＋写真/音声キュー（現状 localStorage のまま。写真/音声は
  実デバイスAPI必須のためこのセッション（ヘッドレスLinux環境・実ブラウザ検証不可）では対象外。
  IndexedDB化自体も現状20件上限でlocalStorage容量は非問題のため優先度は下げて残す）
- 【実装済】復帰同期（2026-09-06。`window`の`online`/`offline`イベント監視による通信状態
  インジケータ＋手動「今すぐ同期」ボタンを追加。既存の同期ロジック（`drainQueue`等、
  純関数・単体テスト済み）はそのまま再利用し、起動時1回のみだった同期トリガーを
  オンライン復帰時にも発火するよう拡張。UI・イベント配線はブラウザ専用のためこの
  セッションでは実ブラウザでの目視確認は実施していない（NOT RUN。build/lint/既存
  単体テストのみで検証）
...
- 【P1】IndexedDB キュー・写真撮影・音声入力（実デバイスAPI・実ブラウザ環境での検証が
  前提のため、ヘッドレスLinux環境で稼働するこのセッションでは対象外。実ブラウザ環境を
  持つ別セッションでの対応を推奨）
- 【実装済】通信/同期状態表示・再同期ボタン（2026-09-06。`navigator.onLine`+
  `online`/`offline`イベントによるオンライン/オフラインインジケータ（保留件数表示込み）+
  手動「🔄 今すぐ同期」ボタンをグローバルヘッダーに追加）
- 【P2】現場位置情報・QR案件呼出し・大型ボタンUI・添付圧縮・写真AI説明・manifest.json/
  Service Worker等のフルPWA化（いずれも実ブラウザ環境での検証が前提のため対象外）
